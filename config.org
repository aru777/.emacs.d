...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package irony
    ...
    (add-hook 'c-mode-hook 'irony-mode)
    ;; irony reads compilation json for each file even though the same
    ;; json has the compile commands for all files.
    ;; When the number of files on the compilation database is huge, it
    ;; takes minutes to open a file. Not good.
